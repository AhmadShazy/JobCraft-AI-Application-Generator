...
function ProfileEdit({ onBackToDashboard }) {
  ...
  if (loading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 transition-colors duration-200">
        <Loader2 className="w-10 h-10 text-primary-500 animate-spin mb-3" />
        <span className="text-xs font-bold tracking-wider text-slate-500 dark:text-slate-400">Loading Profile Details...</span>
      </div>
    );
  }
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 transition-colors duration-200">
      ...
      {showPreviewModal && normalizedPreview && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 dark:bg-slate-950/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 w-full max-w-3xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh] animate-fade-in select-none">
            ...
            <div className="p-6 border-b border-slate-200/60 dark:border-slate-800 flex justify-between items-center flex-shrink-0 bg-slate-50/50 dark:bg-slate-900/50">
              ...
              <button onClick={() => setShowPreviewModal(false)} className="text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 font-bold text-sm transition-colors">Close</button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 text-sm text-slate-800 dark:text-slate-200">
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-cyan-100/60 dark:border-slate-800 rounded-xl space-y-2">
                <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Skills Categorization</h4>
                    ...
                    {Object.entries(normalizedPreview.skills).map(([cat, list]) => (
                      list && list.length > 0 && (
                        <div key={cat} className="flex flex-wrap gap-1.5 items-center">
                          <span className="text-slate-500 dark:text-slate-400 font-bold">{CATEGORY_LABELS[cat] || cat}:</span>
                          {list.map((s, idx) => <span key={idx} className="bg-accent-50 dark:bg-slate-900 border border-accent-100 dark:border-accent-800 text-accent-600 dark:text-accent-400 px-2 py-0.5 rounded-full font-bold">{s}</span>)}
                        </div>
                      )
                    ))}
                  </div>
                ) : (
                  <div className="text-accent-500 dark:text-accent-400 text-xs font-bold">[No Skills Extracted]</div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Experience */}
                <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-cyan-100/60 dark:border-slate-800 rounded-xl space-y-2">
                  <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Work History</h4>
                  {normalizedPreview.experience && normalizedPreview.experience.length > 0 ? (
                    normalizedPreview.experience.map((exp, i) => (
                      <div key={i} className="text-xs space-y-1 border-b border-slate-200/60 dark:border-slate-800 last:border-b-0 pb-2">
                        <div className="font-bold text-slate-800 dark:text-slate-200">{exp.title} at {exp.company}</div>
                        <div className="text-slate-500 dark:text-slate-400">{exp.duration}</div>
                    ...
                    <div className="text-accent-500 dark:text-accent-400 text-xs font-bold">[No Experience Extracted]</div>
                  )}
                </div>

                {/* Projects */}
                <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-cyan-100/60 dark:border-slate-800 rounded-xl space-y-2">
                  <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Projects</h4>
            ...
            {/* Modal Controls */}
            <div className="p-6 border-t border-slate-200/60 dark:border-slate-800 flex justify-end gap-3 flex-shrink-0 bg-slate-50/50 dark:bg-slate-900/50">
              <button onClick={() => setShowPreviewModal(false)} className="px-4 py-2 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 text-slate-600 dark:text-slate-300 rounded-xl text-sm font-semibold transition-all bg-white dark:bg-slate-800">
                Go Back
              </button>
              <button 
