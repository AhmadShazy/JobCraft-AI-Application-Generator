...
function ProfileSetup() {
  ...
  return (
    <div className="h-screen w-full bg-navy-950 text-slate-100 flex flex-col select-none relative overflow-hidden">
      
      {/* Background Accent */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-60 bg-gradient-to-b from-primary-950/20 to-transparent blur-3xl pointer-events-none"></div>

      {/* Header section (fixed at top): Title + Stepper */}
      <div className="w-full max-w-4xl mx-auto px-4 pt-6 pb-2 flex-shrink-0 z-10">
        {/* Title */}
        <div className="text-center mb-4">
          <h1 className="text-2xl font-extrabold bg-gradient-to-r from-primary-300 to-accent-400 bg-clip-text text-transparent flex items-center justify-center gap-2">
            <Sparkles className="w-5.5 h-5.5 text-accent-400 animate-pulse" />
            Candidate Profile setup
          </h1>
          <p className="text-primary-300/70 text-xs mt-0.5">Let's build your professional profile. v2.0</p>
        </div>
              ...
              <div 
                ...
                className={`flex flex-col items-center py-2 rounded-xl border transition-all duration-200 ${
                  active 
                    ...
                    : completed 
                    ? 'bg-primary-950/40 border-primary-800/30 text-emerald-400 shadow-sm shadow-emerald-500/10' 
                    : 'bg-navy-950/40 border-primary-900/20 text-primary-400/20'
                }`}
              >
      ...
      <div className="flex-1 w-full max-w-4xl mx-auto px-4 overflow-y-auto z-10 pb-6 pt-2">
        <div className="bg-navy-900/40 backdrop-blur-xl border border-primary-800/30 rounded-2xl shadow-2xl p-6 md:p-8 min-h-full">
          ...
          {step === 1 && (
            <div className="space-y-6">
              <h2 className="text-lg font-bold border-b border-primary-800/20 pb-2 text-primary-300">Step 1 — Basic Credentials</h2>
              ...
              <div className="space-y-3 pt-2">
                <div className="flex justify-between items-center border-b border-primary-800/20 pb-1">
                  <label className="text-xs font-bold text-primary-400/80">Languages & Proficiency</label>
                ...
                {languages.map((lang, idx) => (
                  <div key={idx} className="flex items-center gap-3 animate-fade-in">
                    <input type="text" value={lang.language} onChange={(e) => handleLanguageChange(idx, 'language', e.target.value)} className="flex-1 py-1.5 px-3 bg-navy-950/60 border border-primary-700/30 rounded-xl text-slate-100 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary-500/60 focus:border-primary-500/40 transition-all" placeholder="e.g. English" />
                    <select value={lang.level} onChange={(e) => handleLanguageChange(idx, 'level', e.target.value)} className="w-40 py-1.5 px-3 bg-navy-950/60 border border-primary-700/30 rounded-xl text-primary-400 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-primary-500/60 focus:border-primary-500/40 transition-all">
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div className="flex justify-between items-center border-b border-primary-800/20 pb-2">
                <h2 className="text-lg font-bold text-primary-300">Step 2 — Education Timeline *</h2>
          ...
          {step === 3 && (
            <div className="space-y-6">
              <h2 className="text-lg font-bold border-b border-primary-800/20 pb-2 text-primary-300">Step 3 — Professional Background Details (Free Text)</h2>
              <p className="text-xs text-primary-400/60 font-semibold">Paste your raw text (from resumes, LinkedIn, drafts) per section. The AI will normalize them into a uniform structure.</p>
              
              <div className="space-y-4">
                <div className="space-y-1">
                  <div className="flex justify-between items-center text-xs font-bold text-primary-300/80">
                    <span>Work Experience</span>
                    <span className="text-primary-400/40">{experience.length} chars</span>
                  </div>
                  <textarea value={experience} onChange={(e) => setExperience(e.target.value)} rows="3" className="w-full py-2.5 px-4 bg-navy-950/60 border border-primary-700/30 rounded-xl text-slate-100 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary-500/60 focus:border-primary-500/40 transition-all" placeholder="Paste your work history here, listing roles, companies, locations, and achievements..."></textarea>
                </div>

                <div className="space-y-1">
                  <div className="flex justify-between items-center text-xs font-bold text-primary-300/80">
                    <span>Skills List</span>
                    <span className="text-primary-400/40">{skills.length} chars</span>
                  </div>
                  <textarea value={skills} onChange={(e) => setSkills(e.target.value)} rows="2" className="w-full py-2.5 px-4 bg-navy-950/60 border border-primary-700/30 rounded-xl text-slate-100 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary-500/60 focus:border-primary-500/40 transition-all" placeholder="List your technical skills, programming languages, databases, or conceptual experience..."></textarea>
                </div>

                <div className="space-y-1">
                  <div className="flex justify-between items-center text-xs font-bold text-primary-300/80">
                    <span>Key Projects</span>
                    <span className="text-primary-400/40">{projects.length} chars</span>
                  </div>
                  <textarea value={projects} onChange={(e) => setProjects(e.target.value)} rows="3" className="w-full py-2.5 px-4 bg-navy-950/60 border border-primary-700/30 rounded-xl text-slate-100 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary-500/60 focus:border-primary-500/40 transition-all" placeholder="List any notable academic, personal, or research projects, including technologies used..."></textarea>
                </div>

                <div className="space-y-1">
                  <div className="flex justify-between items-center text-xs font-bold text-primary-300/80">
                    <span>Certifications</span>
                    <span className="text-primary-400/40">{certifications.length} chars</span>
                  </div>
                  <textarea value={certifications} onChange={(e) => setCertifications(e.target.value)} rows="2" className="w-full py-2.5 px-4 bg-navy-950/60 border border-primary-700/30 rounded-xl text-slate-100 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary-500/60 focus:border-primary-500/40 transition-all" placeholder="E.g., Stanford Machine Learning (Aug 2025), IBM Artificial Intelligence (Jul 2025)..."></textarea>
                </div>

                <div className="space-y-1">
                  <div className="flex justify-between items-center text-xs font-bold text-primary-300/80">
                    <span>Volunteering & Extras</span>
                    <span className="text-primary-400/40">{volunteer.length} chars</span>
                  </div>
                  <textarea value={volunteer} onChange={(e) => setVolunteer(e.target.value)} rows="2" className="w-full py-2.5 px-4 bg-navy-950/60 border border-primary-700/30 rounded-xl text-slate-100 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary-500/60 focus:border-primary-500/40 transition-all" placeholder="Any community work, extracurricular actions, or programming communities you coached..."></textarea>
                </div>

                <div className="space-y-1">
                  <div className="flex justify-between items-center text-xs font-bold text-primary-300/80">
                    <span>Additional Context (Optional)</span>
                    <span className="text-primary-400/40">{additionalInfo.length} chars</span>
                  </div>
                  <textarea value={additionalInfo} onChange={(e) => setAdditionalInfo(e.target.value)} rows="2" className="w-full py-2.5 px-4 bg-navy-950/60 border border-primary-700/30 rounded-xl text-slate-100 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary-500/60 focus:border-primary-500/40 transition-all" placeholder="Any additional context you would like the AI assistant to know about you (e.g. hobbies, preferences)..."></textarea>
                </div>
              </div>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="space-y-6">
              <h2 className="text-lg font-bold border-b border-primary-800/20 pb-2 text-primary-300 flex items-center gap-2">
              ...
              {loading ? (
                <div className="flex flex-col items-center justify-center py-20 space-y-4">
                  ...
                  <div className="text-slate-200 font-bold">Gemini is structuring your profile...</div>
                  <div className="text-xs text-primary-400/60 max-w-xs text-center">Parsing unstructured text and categorizing details. Please hold.</div>
                </div>
              ) : normalizedPreview ? (
                <div className="space-y-6">
                  <p className="text-xs text-primary-400/60 font-semibold">Please review the structured candidate card extracted by Gemini. Any missing sections are highlighted below.</p>
                  
                  {/* Basic Info Preview */}
                  <div className="p-4 bg-navy-950/60 rounded-xl border border-primary-800/20 space-y-2">
                    <h3 className="text-xs font-extrabold uppercase tracking-wider text-primary-400/60">Candidate Information</h3>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div>Name: <span className="font-semibold text-slate-200">{normalizedPreview.name || <span className="text-accent-500 text-xs font-bold">[Missing]</span>}</span></div>
                      <div>Email: <span className="font-semibold text-slate-200">{normalizedPreview.email || <span className="text-accent-500 text-xs font-bold">[Missing]</span>}</span></div>
                      <div>Phone: <span className="font-semibold text-slate-200">{normalizedPreview.phone || <span className="text-accent-500 text-xs font-bold">[Missing]</span>}</span></div>
                      <div>Location: <span className="font-semibold text-slate-200">{normalizedPreview.location || <span className="text-accent-500 text-xs font-bold">[Missing]</span>}</span></div>
                    </div>
                  </div>

                  {/* Skills Categorized Preview */}
                  <div className="p-4 bg-navy-950/60 rounded-xl border border-primary-800/20 space-y-3">
                    <h3 className="text-xs font-extrabold uppercase tracking-wider text-primary-400/60">Categorized Skills</h3>
                        ...
                        {Object.entries(normalizedPreview.skills).map(([category, list]) => (
                          list && list.length > 0 && (
                            <div key={category} className="flex flex-wrap gap-1.5 items-center">
                              <span className="text-primary-400/60 font-bold">{CATEGORY_LABELS[category] || category}:</span>
                              {list.map((s, i) => (
                                <span key={i} className="text-xs px-2 py-0.5 bg-primary-900/40 rounded-full text-primary-300 border border-primary-800/20 font-medium">{s}</span>
                              ))}
                            </div>
                          )
                        ))}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Experience List */}
                    <div className="p-4 bg-navy-950/60 rounded-xl border border-primary-800/20 space-y-2">
                      <h3 className="text-xs font-extrabold uppercase tracking-wider text-primary-400/60">Work Experience</h3>
                      {normalizedPreview.experience && normalizedPreview.experience.length > 0 ? (
                        normalizedPreview.experience.map((exp, idx) => (
                          <div key={idx} className="text-xs space-y-1 pb-2 border-b border-primary-900/40 last:border-b-0">
                            <div className="font-bold text-slate-200">{exp.title} at {exp.company}</div>
                            <div className="text-primary-400/60">{exp.duration} | {exp.location}</div>
                    ...
                    {/* Projects List */}
                    <div className="p-4 bg-navy-950/60 rounded-xl border border-primary-800/20 space-y-2">
                      <h3 className="text-xs font-extrabold uppercase tracking-wider text-primary-400/60">Featured Projects</h3>
                      {normalizedPreview.projects && normalizedPreview.projects.length > 0 ? (
                        normalizedPreview.projects.map((proj, idx) => (
                          <div key={idx} className="text-xs space-y-1 pb-2 border-b border-primary-900/40 last:border-b-0">
                            ...
                            <div className="text-primary-400 font-semibold">Stack: {proj.stack}</div>
                            <ul className="list-disc pl-4 text-slate-300 space-y-0.5 mt-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Certifications */}
                    <div className="p-4 bg-navy-950/60 rounded-xl border border-primary-800/20 space-y-2">
                      <h3 className="text-xs font-extrabold uppercase tracking-wider text-primary-400/60">Certifications</h3>
                      {normalizedPreview.certifications && normalizedPreview.certifications.length > 0 ? (
                        normalizedPreview.certifications.map((c, i) => (
                          <div key={i} className="text-xs text-slate-300">{c.name} — <span className="text-primary-400/60">{c.issuer} ({c.date})</span></div>
                        ))
                      ) : (
                    ...
                    {/* Volunteering */}
                    <div className="p-4 bg-navy-950/60 rounded-xl border border-primary-800/20 space-y-2">
                      <h3 className="text-xs font-extrabold uppercase tracking-wider text-primary-400/60">Volunteer Work</h3>
                      {normalizedPreview.volunteer && normalizedPreview.volunteer.length > 0 ? (
                        normalizedPreview.volunteer.map((v, i) => (
                          <div key={i} className="text-xs space-y-1">
                            ...
                            <div className="text-primary-400/60">{v.duration}</div>
                ...
                <div className="text-center py-20 text-primary-400/60 font-bold">Failed to load normalized profile.</div>
              )}
            </div>
          )}
      ...
      <div className="w-full bg-navy-950/80 backdrop-blur-md border-t border-primary-800/20 py-4 px-4 z-20 flex-shrink-0">
          ...
          <div>
            {step === 1 ? (
              <button
                ...
                disabled={loading}
                className="flex items-center gap-1.5 px-4 py-2 text-sm font-semibold text-primary-400 hover:text-primary-300 transition-colors"
              >
          ...
          <div>
            {step < 4 ? (
              <button
                type="button"
                onClick={nextStep}
                className="flex items-center gap-1.5 px-5 py-2.5 bg-gradient-to-r from-accent-500 to-accent-700 text-navy-950 font-extrabold rounded-xl text-sm shadow-lg shadow-accent-500/20 hover:from-accent-400 hover:to-accent-600 transition-all duration-200"
              >
              ...
              !loading && normalizedPreview && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-sm font-bold shadow-lg shadow-emerald-600/25 transition-all duration-200"
                >
