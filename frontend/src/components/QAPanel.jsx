...
function QAPanel({ qaList, onSendQuestion, isLoading }) {
  ...
  return (
    <div className="flex flex-col h-full bg-cyan-50/15 rounded-2xl border border-cyan-100/60 shadow-sm overflow-hidden">
      
      {/* Panel Header */}
      <div className="p-4 border-b border-cyan-100/60 bg-white/80 flex items-center space-x-2">
        <MessageSquareCode className="w-5 h-5 text-accent-500 animate-pulse" />
        <div>
          <h3 className="font-bold text-slate-800 text-sm">Application Q&A Assistant</h3>
          <p className="text-xs text-slate-500 font-semibold">Type screening questions to get tailored responses</p>
        </div>
      </div>
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="max-w-[85%] flex items-start space-x-2">
              ...
              <div className="bg-white border border-cyan-100/60 text-slate-505 p-3 rounded-2xl rounded-tl-none text-sm font-medium flex items-center space-x-1.5 shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="p-3 border-t border-cyan-100/60 bg-white/80 flex items-center space-x-2">
        <input
          type="text"
          disabled={isLoading}
          value={question}
          ...
          className="flex-1 px-4 py-2.5 bg-white border border-slate-200 focus:border-accent-500 focus:ring-2 focus:ring-accent-100 transition-all rounded-xl text-sm text-slate-800 font-medium placeholder-slate-400 disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={!question.trim() || isLoading}
          className="p-2.5 bg-gradient-to-r from-primary-700 to-accent-500 text-white rounded-xl shadow-sm transition-all duration-200 flex-shrink-0 border border-primary-800/10 hover:from-primary-600 hover:to-accent-600"
        >
